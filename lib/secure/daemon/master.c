...
// update logging to reflect timestamp and reformatted
// error output
// Zed @ Lilypad Mudlib
// 01062013
...
staticf void crash(string, object, object) {
    error("Crash! foo -- Need a more informative error\n-Zed\n");
...
void preload(string file) {
    ...
    if (file_size(file + ".c") == -1)
        return;
    t1 = time();
    write("\nMaster Daemon is Preloading.....\nLoading files from: /etc/preload\nPreloading: " + file + "...");
...
staticf void error_handler(mapping map, int flag) {
  object ob;
  string str;
  string tstamp = OVERRIDES_FILE->timestamp(time());
  string headfoot = "============================================";

  ob = this_interactive() || this_player();
  if (flag) str = "*Error caught\n";
  else str = "";
  str += sprintf(headfoot+"\nError at: "+tstamp+"\n %s\nCurrent object: %O\nCurrent program: %s\nFile: %O Line: %d\n%O\n",
